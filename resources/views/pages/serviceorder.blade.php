					...
					<div class="box ordertask clearfix">
						...
						<div class="ordertask1 edit-product-detail cart-flex clearfix">
							<div class="col-md-6 col-xs-6 edit-product-info">
								<a href="{{ URL::to('service') }}/{{ $service->prod_slug }}/{{ $service->id }}" class="ep-thumb">
									<img src="{{ \App\Products::getFileUrl($service->prod_image) }}" alt="{{ $service->prod_image_alt }}" class="img-responsive">
								</a>
							...
							<div class="col-md-6 col-xs-6 edit-product-task">
								<a href="{{ URL::to('service') }}/{{ $service->prod_slug }}/{{ $service->id }}" class="ep-task">{{ $service->prod_name }}</a>
							</div>
						</div>
						
						<form action="{{ URL::to('serviceordercheckout') }}" method="POST" enctype= multipart/form-data>
						{{ csrf_field() }}
